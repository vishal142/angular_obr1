...
class Degree extends REST_Controller
{
    function __construct(){
        ...
        $this->load->model('api_' . $this->config->item('test_api_ver') . '/admin/degree_model', 'degree');
    ...
    * 
    */
    public function getAllDegree_post()
    {
        ...
        else 
        {
            ...
            if($flag)
            {
                ...
                {
                    $details_arr['dataset']     = $this->degree->getAllDegree($req_arr);
                    $count                      = $this->degree->getAllDegreeCount($req_arr);
                    $details_arr['count']       = $count['count_degree'];
                    //pre($details_arr,1);
    ...
    * 
    */
    public function addDegree_post()
    {
        $error_message = $success_message = $http_response = '';
        $result_arr = array();
        if (!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())) 
        ...
        else 
        {
            $req_arr = $details_arr = array();
            $flag = true;
            if(empty($this->post('degree_name')))
            {
                $flag = false;
            ...
            {
                $req_arr['degree_name'] = $this->post('degree_name');
            } 

            if($flag)
            {
                $checkDegree = $this->degree->checkDuplicateDegree($req_arr);
                //echo $this->db->last_query();
                if(empty($checkDegree))
                {
                    $degreeId = $this->degree->addDegree($req_arr);
                    if(!empty($degreeId))
                    {
                        $http_response      = 'http_response_ok';
                        $success_message    = 'Add degree successfully';  
                    }
                    else
                    {
                        $http_response  = 'http_response_bad_request';
                ...
                {
                    $http_response = 'http_response_bad_request';
                    $error_message = 'Degree Name already exists, please try another name';
                }
            }
            else 
            {
    ...
    * 
    */
    public function getDegreeDetail_post()
    {
        $error_message = $success_message = $http_response = '';
        $result_arr = array();
        if (!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())) 
        ...
        else 
        {
            ...
            if($flag)
            {
                $degreeDetails = $this->degree->getDegreeDetailById($req_arr);
    ...
    /*
    * --------------------------------------------------------------------------
    * @ Function Name            : updateDegreeDetail()
    * @ Added Date               : 08-09-2016
    * @ Added By                 : Piyalee
    * -----------------------------------------------------------------
    ...
    * 
    */
    public function updateDegreeDetail_post()
    {
        $error_message = $success_message = $http_response = '';
        $result_arr = array();
        if (!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())) 
        ...
        else 
        {
            ...
            if($flag)
            {
                ...
                if(empty($checkDegree))
                {
                    $degreeId = $this->degree->updateDegree(array('id' => $req_arr['degree_id']), array('degree_name' => $req_arr['degree_name']));
                    
                    $http_response      = 'http_response_ok';
                    $success_message    = 'Updated degree successfully';
                }
                else
                {
                    $http_response = 'http_response_bad_request';
